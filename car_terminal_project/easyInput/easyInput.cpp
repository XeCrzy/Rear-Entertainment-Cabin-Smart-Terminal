...
easyInput::easyInput(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::easyInput),
    fatherObject(parent),
    m_initTimer(nullptr),
    m_canShow(false)
{
    ...
    connect(m_initTimer, &QTimer::timeout, this, &easyInput::onInitTimeout);
    m_initTimer->start(1000);  // 1秒后初始化
}
...
{
    QString absolutePath = "/piyin";
    py.open(absolutePath);
}
...
void easyInput::setFontInfo(const QString &fontName, int btnFontSize, int labFontSize)
{
    ...
    foreach (QPushButton *btn, btns) {
        // --- 新增：彻底解决黑字问题的关键代码 ---
        btn->setPalette(QPalette());          // 重置调色板
        btn->setAutoFillBackground(false);    // 禁止自动填充背景
