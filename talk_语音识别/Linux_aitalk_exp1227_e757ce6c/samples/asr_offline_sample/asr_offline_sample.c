...
int build_grammar(UserData *udata)
{
	...
	char grm_build_params[MAX_PARAMS_LEN]    = {0};  // 修复: 改为 {0}
	int ret                                  = 0;
...
int update_lexicon(UserData *udata)
{
	...
	char update_lex_params[MAX_PARAMS_LEN]    = {0};  // 修复: 改为 {0}
...
{
	char asr_params[MAX_PARAMS_LEN]    = {0};  // 修复: 改为 {0}
	...
	if (NULL != rec_rslt)
{
    ...
    // 提取<rawtext>标签内容
    char *start = strstr(rec_rslt, "<rawtext>");
    char *end = strstr(rec_rslt, "</rawtext>");
    
    if(start && end) {
        start += 9; // Skip "<rawtext>" length
        int len = end - start;
        char command[256];
        ...
        send(conn_fd, command, strlen(command), 0);
        printf("Send command: %s\n", command);
    } else {
        send(conn_fd, "Recognition failed", 18, 0);
    }
}
	else
	{
    	printf("No recognition result!\n");
    	send(conn_fd, "No recognition result", 21, 0);
	}
	printf("=============================================================\n");
...
int main(int argc, char* argv[])
{
	...
	const char *login_config    = "appid = e757ce6c"; //登录参数
	UserData asr_data; 
	int ret                     = 0;
	...
	//IP地址
	srvaddr.sin_addr.s_addr = inet_addr("192.168.16.181");
	...
	while(1)
	{
		...
		file_size = ntohl(file_size);  // 关键修复：字节序转换
		printf("file_size = %d\n", file_size);
