...
// 发送规范化命令到服务器
bool client_c_send_control_command(const char* device, int state)
{
    if (!client_c_is_connected()) {
        printfLog(EN_LOG_LEVEL_WARNING, "Client_C: 无法发送控制命令，未连接\n");
        return false;
    }
    
    if (device == NULL) {
        printfLog(EN_LOG_LEVEL_ERROR, "Client_C: 设备名不能为空\n");
        return false;
    }
    ...
    if (strcmp(device, "LED") == 0) {
        if (state == 0) {
            snprintf(command, sizeof(command), "LED_OFF");
        } else {
            snprintf(command, sizeof(command), "LED_ON");
        }
    } else if (strcmp(device, "BUZZER") == 0) {
        if (state == 0) {
            snprintf(command, sizeof(command), "BUZZER_OFF");
        } else {
            snprintf(command, sizeof(command), "BUZZER_ON");
        }
    } else {
        printfLog(EN_LOG_LEVEL_ERROR, "Client_C: 未知设备: %s\n", device);
        return false;
    }
    
    printfLog(EN_LOG_LEVEL_INFO, "Client_C: 发送控制命令: %s\n", command);
    return client_c_send_command(command);
}

// 发送状态报告（可选，如果需要报告状态给服务器）
bool client_c_send_status_report(int led_status, int buzzer_status)
{
    if (!client_c_is_connected()) {
        return false;
    }
    ...
    char status[64];
    snprintf(status, sizeof(status), "STATUS:LED:%d, BUZZER:%d", led_status, buzzer_status);
    
    return client_c_send_command(status);
}